...
function main() {
  ...
  if (typeof report.npmMajor === 'number' && report.npmMajor >= 12) {
    ...
    if (!/dry-run/.test(rev.source)) {
      failed = true;
      console.error(`::error::review did not use the npm v12 dry-run path on npm v${report.npmMajor} (source: ${rev.source}) — the pending-set parser may be out of date`);
    } else if (!rev.pending.some((p) => p.name === 'core-js')) {
      failed = true;
      console.error(`::error::npm v${report.npmMajor} dry-run did NOT report the planted scripted dep core-js as pending — its pending list shape likely changed (renamed key?)`);
    }
  }
