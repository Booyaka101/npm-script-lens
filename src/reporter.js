...
// audit --runtime: payload-shaped signals in the code a package runs when it
// is required, which no install-script audit reads.
...
function buildReport(results, { note } = {}) {
  ...
  lines.push(...runtimePayloadSection(results));
...
// audit --runtime. error at HIGH (exfil, or two payload kinds together),
// warning at MEDIUM.
const RUNTIME_PAYLOAD_RULE = {
  id: 'runtime-payload',
  text: 'Runtime code (main/exports/bin) names a C2 or exfiltration endpoint, starts node on a file the package ships, or is an obfuscator.io string-array payload, the btree loader pattern: it runs when the package is used, with no install script',
};
...
function buildSarif(results, { lockPath = 'package-lock.json', lockText = '', findings = [] } = {}) {
  ...
  for (const r of results) {
    ...
    if (r.runtimePayload) {
      gypSarifResults.push({
        ruleId: RUNTIME_PAYLOAD_RULE.id,
        level: r.runtimePayload.risk === 'HIGH' ? 'error' : 'warning',
        message: { text: `${r.name}@${r.version}: ${RUNTIME_PAYLOAD_RULE.text}. ${r.runtimePayload.hits.map((h) => `${h.signal} (${h.files.join(', ')})`).join(' | ')}` },
        locations: [{ physicalLocation: { artifactLocation: { uri }, region: { startLine: lineOf(r.name) } } }],
        partialFingerprints: { runtimePayload: `runtime-payload:${r.name}@${r.version}` },
      });
    }
    ...
    runs: [{
      tool: {
        driver: {
          ...
          rules: [...Object.values(SARIF_RULES), GYP_RULE, RUNTIME_BOOTSTRAP_RULE, RUNTIME_PAYLOAD_RULE, ...Object.values(GAP_RULES), PUBLISH_RULE, PUBLISH_OIDC_RULE, ...Object.values(PUBLISH_GATE_RULES), HOOK_RULE, ...Object.values(PROVENANCE_RULES), TRUST_DOWNGRADE_RULE, COOLDOWN_CONFIG_RULE].map((rule) => ({
            id: rule.id,
...
function buildHtml(results, { note, title = 'npm-script-lens report' } = {}) {
  ...
  const payloadRows = payloads.map((r) => {
    ...
    return `<tr><td><strong>${htmlEsc(r.name)}</strong>@${htmlEsc(r.version)}</td><td><span class="badge" style="background:${color}">${label}</span></td><td>${hits}</td></tr>`;
  }).join('\n');
  const payloadHtml = payloads.length === 0 ? ''
    : `<h2 style="font-size:16px">Runtime payload</h2>\n<table><thead><tr><th>package</th><th>risk</th><th>runtime-code signals</th></tr></thead><tbody>\n${payloadRows}\n</tbody></table>`;
